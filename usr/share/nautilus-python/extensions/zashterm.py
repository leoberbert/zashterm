...
"""
Zashterm - Nautilus Extension
Adds context menu options to open Zashterm. Supports local paths,
...
try:
    ...
    print(f"Zashterm Extension: Could not set up localization: {e}")

_ = gettext.gettext

# --- Constants ---
TERMINAL_EXECUTABLE = shutil.which("zashterm") or shutil.which("zash")
REMOTE_URI_SCHEMES = {"sftp", "ssh"}


class ZashinalExtension(GObject.GObject, Nautilus.MenuProvider):
    """
    Provides context menu items for opening directories in Zash.
    """
    ...
    def _launch(self, command: list[str]):
        ...
        if not TERMINAL_EXECUTABLE:
            print("Zashterm Error: 'zashterm' executable not found.")
            return
        try:
    ...
    def _launch_local_session(self, menu_item: Nautilus.MenuItem, files: list[Nautilus.FileInfo]):
        """
        Opens Zashterm in the specified local or GVFS directory path.
        """
        file = files[0]
        local_path = self._get_local_path(file)
        if not local_path:
            print(f"Zashterm Error: Could not get local path for {file.get_uri()}")
            return
    ...
    def _launch_remote_ssh_session(self, menu_item: Nautilus.MenuItem, files: list[Nautilus.FileInfo]):
        """
        Parses a remote URI and launches Zashterm with a direct SSH connection.
        """
        file = files[0]
        uri = file.get_uri()
        try:
            ...
            self._launch(cmd)
            print(f"Zashterm: Launched SSH session to {target}")
    ...
    def _get_menu_items(self, files: list[Nautilus.FileInfo]) -> list[Nautilus.MenuItem]:
        ...
        if is_remote:
            ...
            ssh_item = Nautilus.MenuItem(
                name="Zashinal::OpenRemoteSSH",
                label=_("Open in Zashterm (SSH)"),
            ...
            if local_path:
                gvfs_item = Nautilus.MenuItem(
                    name="Zashinal::OpenGVFS",
                    label=_("Open in Zashterm"),
        ...
        # Case 2: Local location (file://)
            ...
            local_item = Nautilus.MenuItem(
                name="Zashinal::OpenLocal",
                label=_("Open in Zashterm"),
                tip=_("Open {} in Zashterm").format(file.get_name()),
            )
