...
from .exceptions import ZashinalError
from .logger import get_logger

# Schema to identify Zash passwords in the system keyring.
SECRET_SCHEMA = Secret.Schema.new(
    "io.github.leoberbert.zashterm.Password",
...
def store_password(session_name: str, password: str) -> bool:
    ...
    if not is_encryption_available():
        raise ZashinalError("Secret Service API is not available.")
    ...
    try:
        ...
        Secret.password_store_sync(
            ...
            f"Password for Zash session '{session_name}'",
            password,
            None,
        )
        ...
        raise ZashinalError(f"Failed to store password: {e}") from e
...
def lookup_password(session_name: str) -> Optional[str]:
    ...
    try:
        ...
        raise ZashinalError(f"Failed to lookup password: {e}") from e
...
def export_all_passwords(sessions_store: Gio.ListStore) -> Dict[str, str]:
    ...
    logger = get_logger("zashterm.crypto")
    passwords = {}
    if not is_encryption_available():
