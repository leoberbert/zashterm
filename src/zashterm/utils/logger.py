...
class LoggerConfig:
    ...
    def __init__(self):
        primary_log_dir = Path.home() / ".config" / "zashterm" / "logs"
        legacy_log_dir = Path.home() / ".config" / "zash" / "logs"
        self.log_dir = legacy_log_dir if not primary_log_dir.exists() and legacy_log_dir.exists() else primary_log_dir
        self.log_dir.mkdir(parents=True, exist_ok=True)
        self.main_log_file = self.log_dir / "zashterm.log"
        self.error_log_file = self.log_dir / "zashterm_errors.log"
...
class LoggerManager:
    ...
    def enable_debug_mode(self):
        ...
        os.environ["ZASH_DEBUG"] = "1"

    def disable_debug_mode(self):
        self.set_console_level(LogLevel.INFO)
        os.environ.pop("ZASH_DEBUG", None)
...
def log_app_start():
    ...
    logger = get_logger("zashterm.startup")
    logger.info("Zash starting up")
    cleanup_old_logs()


def log_app_shutdown():
    """Log application shutdown."""
    logger = get_logger("zashterm.shutdown")
    logger.info("Zash shutting down")


def log_terminal_event(event_type: str, terminal_name: str, details: str = ""):
    """Log terminal-related events."""
    logger = get_logger("zashterm.terminal")
...
def log_session_event(event_type: str, item_name: str, details: str = ""):
    ...
    logger = get_logger("zashterm.sessions")
