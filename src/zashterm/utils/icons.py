# zash/utils/icons.py
"""
Icon loading utilities for Zash.

Provides icon loading from bundled SVG files with proper symbolic icon
color adaptation via GTK's icon rendering system.

Usage:
    from zashterm.utils.icons import icon_button, icon_image

    # These automatically use bundled icons when setting is 'zashterm'
...
_ICON_PATHS = [
    # Primary (new name)
    "/usr/share/zashterm/icons",
    # Legacy location kept for compatibility with existing installs
    "/usr/share/zash/icons",
    # Editable source tree
    str(Path(__file__).parent.parent / "icons"),  # src/zashterm/icons
]
