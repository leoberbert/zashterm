...
class ZashinalError(Exception):
    """Base exception class for all Zash errors."""
...
class TerminalError(ZashinalError):
...
class SessionError(ZashinalError):
...
class StorageError(ZashinalError):
...
class ConfigError(ZashinalError):
...
class ValidationError(ZashinalError):
...
class PermissionError(ZashinalError):
...
def handle_exception(
    ...
    reraise: bool = False,
) -> Optional[ZashinalError]:
    """Handle an exception by logging it and optionally converting to ZashinalError."""
    from .logger import log_error_with_context

    log_error_with_context(exception, context, logger_name)
    converted_exception = (
        exception
        if isinstance(exception, ZashinalError)
        else ZashinalError(
