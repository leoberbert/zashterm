...
class PlatformInfo:
    ...
    def __init__(self):
        self.logger = get_logger("zashterm.platform")
    ...
    def _get_config_directory(self) -> Path:
        ...
        if xdg_config := os.environ.get("XDG_CONFIG_HOME"):
            primary = Path(xdg_config) / "zashterm"
            legacy = Path(xdg_config) / "zash"
        else:
            primary = self.home_dir / ".config" / "zashterm"
            legacy = self.home_dir / ".config" / "zash"
    ...
    def _get_cache_directory(self) -> Path:
        ...
        if xdg_cache := os.environ.get("XDG_CACHE_HOME"):
            primary = Path(xdg_cache) / "zashterm"
            legacy = Path(xdg_cache) / "zash"
        else:
            primary = self.home_dir / ".cache" / "zashterm"
            legacy = self.home_dir / ".cache" / "zash"
...
class PathManager:
    ...
    def __init__(self, platform_info: PlatformInfo):
        ...
        self.logger = get_logger("zashterm.platform.paths")
...
def ensure_directory_exists(directory: Union[str, Path], mode: int = 0o755) -> bool:
    ...
    try:
        ...
        logger = get_logger("zashterm.platform.directory")
