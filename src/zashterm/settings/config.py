...
class AppConstants:
    ...
    APP_ID = "io.github.leoberbert.zashterm"
    APP_TITLE = "Zash Terminal"
    APP_VERSION = "1.8.4"
    DEVELOPER_NAME = "Zashterm Project"
    DEVELOPER_TEAM = ["Zashterm Maintainers"]
    COPYRIGHT = "© 2025 Zashterm"
    WEBSITE = "https://github.com/leoberbert/zashterm"
    ISSUE_URL = "https://github.com/leoberbert/zashterm/issues"
...
class ConfigPaths:
    ...
    def __init__(self):
        self.logger = get_logger("zashterm.config.paths") if UTILS_AVAILABLE else None
        self._setup_paths()
    ...
    def _setup_paths(self):
        try:
            ...
            legacy_dir = self._get_legacy_config_dir()
            if not self.CONFIG_DIR.exists() and legacy_dir.exists():
    ...
    def _get_default_config_dir(self) -> Path:
        if xdg_config := os.environ.get("XDG_CONFIG_HOME"):
            return Path(xdg_config) / "zashterm"
        return Path.home() / ".config" / "zashterm"

    def _get_legacy_config_dir(self) -> Path:
        if xdg_config := os.environ.get("XDG_CONFIG_HOME"):
            return Path(xdg_config) / "zash"
        return Path.home() / ".config" / "zash"

    def _get_cache_directory(self) -> Path:
        if xdg_cache := os.environ.get("XDG_CACHE_HOME"):
            return Path(xdg_cache) / "zashterm"
        return Path.home() / ".cache" / "zashterm"

    def _use_fallback_paths(self):
        home = Path.home()
        self.CONFIG_DIR = home / ".config" / "zashterm"
        self.SESSIONS_FILE = self.CONFIG_DIR / "sessions.json"
        self.SETTINGS_FILE = self.CONFIG_DIR / "settings.json"
        self.STATE_FILE = self.CONFIG_DIR / "session_state.json"
        self.LAYOUT_DIR = self.CONFIG_DIR / "layouts"
        self.CACHE_DIR = home / ".cache" / "zashterm"
        self.LOG_DIR = self.CONFIG_DIR / "logs"
...
class DefaultSettings:
    ...
    def get_defaults() -> Dict[str, Any]:
        return {
            ...
            # Icon Theme Strategy: "zashterm" (bundled) or "system"
            # Using bundled icons by default speeds up GTK4 startup
            "icon_theme_strategy": "zashterm",
            # Shortcuts
            "shortcuts": {
...
def initialize_configuration():
    ...
    logger = get_logger("zashterm.config") if UTILS_AVAILABLE else None
    try:
        if logger:
            logger.info(
...
try:
    ...
    CONFIG_DIR = os.path.expanduser("~/.config/zashterm")
