...
def main() -> int:
    ...
    logger = get_logger("zashterm.main")

    # Tries to set the process title and logs failures
    try:
        import setproctitle

        setproctitle.setproctitle("zashterm")
        logger.info("Process title set to 'zashterm'.")
    except Exception as e:
    ...
    parser = argparse.ArgumentParser(
        prog="zashterm",
        description=_(
            "Zashterm - A modern terminal emulator with session management"
        ),
        epilog=_(
            "For more information, visit: https://github.com/leoberbert/zashterm"
        ),
    )
    ...
    try:
        ...
        alert.set_detail(_("Could not start Zashterm.\n\nError: {}").format(e))
