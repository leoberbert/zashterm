...
class FileManager(GObject.Object):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.logger = get_logger("zashterm.filemanager.manager")
        ...
        if self.settings_manager.get("use_system_tmp_for_edit", False):
            self.remote_edit_dir = Path(tempfile.gettempdir()) / "zash_remote_edit"
            self.logger.info(
    ...
    def _open_and_monitor_local_file(
        ...
        if app:
            notification = Gio.Notification.new(_("Zash"))
            ...
            app.send_notification(f"zashterm-file-open-{remote_path}", notification)
    ...
    def _on_save_upload_complete(self, transfer_id, success, message):
        ...
        notification.set_icon(Gio.ThemedIcon.new("utilities-terminal-symbolic"))
        app.send_notification(f"zashterm-upload-complete-{transfer_id}", notification)
