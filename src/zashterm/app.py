...
if TYPE_CHECKING:
    from .window import ZashWindow


class ZashApp(Adw.Application):
    """Main application class for Zash."""
    ...
    def __init__(self):
        ...
        self.logger = get_logger("zashterm.app")
        self.logger.info(f"Initializing {APP_TITLE} v{APP_VERSION}")

        self.settings_manager: Optional[SettingsManager] = None
        self._main_window: Optional["ZashWindow"] = None
    ...
    def _initialize_subsystems(self) -> bool:
        ...
        try:
            ...
            handle_exception(
                e, "application initialization", "zashterm.app", reraise=True
            )
            return False
    ...
    def _configure_icon_theme(self) -> None:
        ...
        When 'zashterm' strategy is selected, icons are loaded directly from
        ...
        """
        try:
            icon_strategy = "zashterm"
            if self.settings_manager:
                icon_strategy = self.settings_manager.get(
                    "icon_theme_strategy", "zashterm"
                )
                # Legacy value migration (pre-rename)
                if icon_strategy == "zash":
                    icon_strategy = "zashterm"
                    self.settings_manager.set("icon_theme_strategy", icon_strategy)

            # Configure icons module based on strategy
            from .utils import icons

            icons._use_bundled_icons = icon_strategy == "zashterm"

            self.logger.info(
                f"Icon strategy: {'Zashterm (bundled SVGs)' if icon_strategy == 'zashterm' else 'System icons'}"
            )
    ...
    def _on_backup_now_action(self, _action, _param) -> None:
        ...
        file_dialog.set_initial_name(f"zashterm-backup-{timestamp}.tar.gz")
    ...
    def _show_restore_success_dialog(self):
        dialog = Adw.MessageDialog(
            ...
            body=_(
                "Data has been restored successfully. Please restart Zash for the changes to take effect."
            ),
            close_response="ok",
        )
    ...
    def create_new_window(self, **kwargs) -> "ZashWindow":
