# zash/ui/dialogs/preferences_dialog.py
...
class PreferencesDialog(Adw.PreferencesWindow):
    ...
    def __init__(self, parent_window, settings_manager: SettingsManager):
        ...
        self.logger = get_logger("zashterm.ui.dialogs.preferences")
    ...
    def _setup_appearance_page(self) -> None:
        ...
        icon_theme_row.set_model(
            Gtk.StringList.new([_("Zashterm Icons (Bundled)"), _("System Icons")])
        )
        current_strategy = self.settings_manager.get("icon_theme_strategy", "zashterm")
        if current_strategy == "zash":  # legacy value from old name
            current_strategy = "zashterm"
        icon_strategy_index = 0 if current_strategy == "zashterm" else 1
    ...
    def _on_icon_theme_changed(self, combo_row, _param) -> None:
        ...
        """
        index = combo_row.get_selected()
        strategy = "zashterm" if index == 0 else "system"
